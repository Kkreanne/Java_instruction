...
public class ProductTextFile implements DAO<Product> {
	...
	private final String PRODUCTS_URL = "c:/bootcamp/java/fileiodemo/products.txt";
	
	public ProductTextFile() {
		productsPath = Paths.get(PRODUCTS_URL);
	...
	private boolean saveAll() {
		try (PrintWriter out = new PrintWriter(
			...
			for (Product p : products) {	//for every product in Products list, write it to our file
	...
	@Override
	public boolean update(Product newProduct) {	//replacing a product
		Product oldProduct = this.get(newProduct.getCode());
		int i = products.indexOf(oldProduct);
		products.remove(i); //remove old
		products.add(i, newProduct); //add new at the same index
		return this.saveAll();
	}
